...
class FormTest(APITestCase):
    ...
    form_data = {'name': 'Test', 'url': 'https://test.com', 'test_period': 5}
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(pk=1,
                                             email='test@user.com',
                                             password='foo')
        self.alt_user = User.objects.create_user(pk=2,
                                                 email='testalt@user.com',
                                                 password='foo')
    ...
    def get_submission_url(self, pk):
        return reverse('sub-api:submission-list', kwargs={'form_pk': pk})

    def get_submission_detail_url(self, form_pk, sub_pk):
        return reverse('sub-api:submission-detail',
                       kwargs={
                           'form_pk': form_pk, 'pk': sub_pk
                       })
    ...
    def create_submission(self):
        id = self.create_form()
        response = self.client.post(self.get_submission_url(id),
    ...
    def test_create_same_name_url(self):
        ...
        if self.assertIn('name', response.data, msg='Name error not found'):
            self.assertEqual(
                ...
                'You have already created a form with that name')

        if self.assertIn('url', response.data, msg='URL error not found'):
            self.assertEqual(
                response.data.get('url')[0],
                'You have already created a form with that url')
    ...
    def test_create_form_with_neg_time_period(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data.get('test_period')[0],
    ...
    def test_patch_form(self):
        ...
        response = self.client.patch(self.get_form_detail_url(id),
                                     data={'name': 'New'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_put_form(self):
        alt_form_data = {
            'name': 'Alt test', 'url': 'https://alt.com', 'test_period': 5
        }

        id = self.create_form()
        response = self.client.put(
            self.get_form_detail_url(id),
            data=alt_form_data) # change data to alt data
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], 'Alt test')
        self.create_form()

        # url is same as prev breaking unique integrity
        response = self.client.put(self.get_form_detail_url(id),
                                   data=self.form_data)
    ...
    def test_get_submission_from_form(self):
        id = self.create_form()
        self.client.post(self.get_submission_url(id),
                         data=self.submission_data)
    ...
    def test_retrieve_submission(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.submission_data['key'], response.data.get('key'))
    ...
    def test_edit_submission(self):
        ...
        response = self.client.patch(self.get_submission_detail_url(
            id, sub_pk),
                                     data={'key': 'key2'})
...
'''
    def test_multiple_errors(self):
        Form.objects.create(
            name='Test', url='https://test.com', test_period=1, created_by=self.user)
        try:
